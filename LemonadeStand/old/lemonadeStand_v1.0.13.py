...
def saveGame():
    ...
    #tmpStr = hexi(tmpStr)
    print(tmpStr)
    #tmpStr2= hexi(tmpStr2)
    print(tmpStr2)
    #tmpStr3= hexi(tmpStr3)
    print(tmpStr3)
...
while login:
    ...
    while not os.path.exists('data/'+user+__version__+'.txt'):
        ...
        with open('data/'+user+__version__+'.txt','r') as file:
            ...
            print(str(database))
            coreData=database[0];coreTag=coreData[:7];coreData[7:]
            print(str(coreTag));print('COREDATA: '+str(coreData))
            adData=database[1];adTag=adData[:5];adData=adData[5:]
            print(str(adTag));print('ADTAG: '+str(adData))
            loanData=database[2];loanTag=loanData[:7];loanData=loanData[7:]
            print(str(loanTag));print('LOANTAG: '+str(loanTag))
            #coreData
            data=coreData;data=decode(data);lineList=data.split(',');dataList=[]
            ...
            #adData
            if adTag=='adtag':
                data=adData;data=decode(data);lineList=data.split(',');dataList=[]
                for line in lineList:
                    dataList.append(line)
                print('ad: '+str(dataList))
                if (len(dataList)/4)>=1:
                    for ct in range(0,(len(dataList)/4)):
                        tmpAd=[]
                        tmpAd=[dataList[(ct*4)],dataList[(ct*4)+1],dataList[(ct*4)+2],dataList[(ct*4)+3]]
                        curAds.append(tmpAd)
            #loanData
            if loanTag=='loantag':
                data=loanData;data=decode(data);lineList=data.split(',');dataList=[]
                for line in lineList:
                    dataList.append(line)
                print('loan: '+str(dataList))
                dataList.pop(0)
                if (len(dataList)/3)>=1:
                    print(len(dataList)/3)
                    for ct in range(0,(int(math.floor(len(dataList)/3)))):
                        tmpLoan=[]
                        tmpLoan=[dataList[(ct*4)],dataList[(ct*4)+1],dataList[(ct*4)+2]]
